...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("GET /api/v1/billing/wallets", s.listWallets)
	...
	s.mux.HandleFunc("POST /api/v1/fabric/resource-preflight", s.createHold)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-create-failed", s.releaseHolds)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-created", s.recordEvidenceRecord)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-usage-tick", s.recordResourceUsage)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-settlement", s.settleWorkspaceUsage)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-stopped", s.releaseHolds)
	s.mux.HandleFunc("POST /api/v1/fabric/resource-destroyed", s.releaseHolds)
...
func (s *Server) listWallets(w http.ResponseWriter, r *http.Request) {
	...
	wallets, err := s.store.ListWallets(r.Context(), walletFilterFromQuery(r))
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
...
func walletFilterFromQuery(r *http.Request) ledger.WalletFilter {
	q := r.URL.Query()
	return ledger.WalletFilter{
		AccountID: q.Get("accountId"),
		UserID:    q.Get("userId"),
	}
}
