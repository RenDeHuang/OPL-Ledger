...
type Store interface {
	...
	ListResourceUsage(context.Context, ResourceUsageFilter) ([]usage.ResourceUsageLog, error)
...
func (s *PostgresStore) RecordRequestUsage(ctx context.Context, input RequestUsageInput) (RequestUsageResult, error) {
	...
	var nextQuota *usage.RequestQuota
	if input.RequestQuota != nil {
	...
	createdAt := nowUTC()
	if input.RequestQuota == nil {
		nextQuota, err = s.incrementPersistedRequestQuota(ctx, tx, accountID, input.UserID, input.WorkspaceID, createdAt)
		if err != nil {
			return RequestUsageResult{}, err
		}
	}
...
func (s *PostgresStore) UpsertRequestQuota(ctx context.Context, input RequestQuotaInput) (RequestQuotaRecord, error) {
	...
	payload, err := json.Marshal(record.Quota)
	if err != nil {
		return RequestQuotaRecord{}, err
	}
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO request_quotas (
			id, account_id, user_id, workspace_id, quota, created_at, updated_at
		) VALUES (
			$1, $2, $3, $4, $5, $6, $7
		)
		ON CONFLICT (account_id, user_id, workspace_id) DO UPDATE SET
...
func (s *PostgresStore) ListRequestQuotas(ctx context.Context, filter RequestQuotaFilter) ([]RequestQuotaRecord, error) {
	where, args := requestQuotaWhere(filter)
	query := `SELECT id, account_id, user_id, workspace_id, quota, created_at, updated_at FROM request_quotas`
	if where != "" {
		query += ` WHERE ` + where
	}
	query += ` ORDER BY account_id, user_id, workspace_id`
	rows, err := s.db.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	return scanRequestQuotaRecords(rows)
}
...
func (s *PostgresStore) ListManualTopUps(ctx context.Context, filter ManualTopUpFilter) ([]ManualTopUp, error) {
	where, args := manualTopUpWhere(filter)
	query := `SELECT payload FROM manual_topups`
	if where != "" {
		query += ` WHERE ` + where
	}
	query += ` ORDER BY created_at, id`
...
func (s *PostgresStore) ListResourceUsage(ctx context.Context, filter ResourceUsageFilter) ([]usage.ResourceUsageLog, error) {
	where, args := resourceUsageWhere(filter)
	query := `SELECT payload FROM resource_usage_logs`
	if where != "" {
		query += ` WHERE ` + where
	}
	query += ` ORDER BY created_at, id`
	...
	defer rows.Close()
	return scanResourceUsageLogs(rows)
}
...
func (s *PostgresStore) incrementPersistedRequestQuota(ctx context.Context, tx *sql.Tx, accountID string, userID string, workspaceID string, now time.Time) (*usage.RequestQuota, error) {
	if userID == "" {
		return nil, nil
	}
	row := tx.QueryRowContext(ctx, `
		SELECT id, account_id, user_id, workspace_id, quota, created_at, updated_at
		FROM request_quotas
		WHERE account_id = $1 AND user_id = $2 AND workspace_id = $3
		FOR UPDATE`,
		accountID,
		userID,
		workspaceID,
	)
	record, err := scanRequestQuotaRecord(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
...
func scanResourceUsageLogs(rows *sql.Rows) ([]usage.ResourceUsageLog, error) {
	var logs []usage.ResourceUsageLog
	for rows.Next() {
		var payload []byte
		if err := rows.Scan(&payload); err != nil {
			return nil, err
...
func scanRequestQuotaRecords(rows *sql.Rows) ([]RequestQuotaRecord, error) {
	...
	for rows.Next() {
		record, err := scanRequestQuotaRecord(rows)
		if err != nil {
			return nil, err
		}
...
func scanRequestQuotaRecord(scanner interface {
	Scan(dest ...any) error
}) (RequestQuotaRecord, error) {
	var record RequestQuotaRecord
	var payload []byte
	err := scanner.Scan(
		&record.ID,
		&record.AccountID,
	...
	if len(payload) > 0 {
		if err := json.Unmarshal(payload, &record.Quota); err != nil {
			return RequestQuotaRecord{}, err
		}
	}
	return record, nil
...
func scanManualTopUps(rows *sql.Rows) ([]ManualTopUp, error) {
...
func resourceUsageWhere(filter ResourceUsageFilter) (string, []any) {
	var clauses []string
	var args []any
	add := func(column string, value string) {
		if value == "" {
	...
	add("storage_id", filter.StorageID)
	add("attachment_id", filter.AttachmentID)
	add("resource_kind", string(filter.ResourceKind))
	add("source_event_id", filter.SourceEventID)
	return strings.Join(clauses, " AND "), args
}

func manualTopUpWhere(filter ManualTopUpFilter) (string, []any) {
	var clauses []string
	var args []any
	add := func(column string, value string) {
		if value == "" {
	...
	add("account_id", filter.AccountID)
	add("user_id", filter.UserID)
	add("operator_id", filter.OperatorUserID)
...
func requestQuotaWhere(filter RequestQuotaFilter) (string, []any) {
	var clauses []string
	var args []any
	add := func(column string, value string) {
		if value == "" {
